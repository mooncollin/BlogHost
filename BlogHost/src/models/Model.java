		...
		if(changed)
		{
			...
			if(connection != null)
			{
				try
				{
					...
					if(results.next())
					{
						for(Field field : fields.stream().filter(field -> !field.getAnnotation(Column.class).autoGenerated()).collect(Collectors.toList()))
						{
							results.updateObject(field.getAnnotation(Column.class).columnName(), field.get(this));
						}
						results.updateRow();
					}
					else
